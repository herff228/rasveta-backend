...
const generateAllTasks = async () => {
  ...
  if (!process.env.YANDEX_API_KEY || !process.env.YANDEX_FOLDER_ID) {
    console.log('⚠️ Yandex GPT не настроен. Использую задания по умолчанию');
    ...
    const response = await axios.post(
      'https://llm.api.cloud.yandex.net/foundationModels/v1/completion',
      {
        ...
        messages: [
          {
            role: 'user',
            text: `Придумай 9 разных заданий для саморазвития. 
Для каждого задания напиши КОРОТКОЕ НАЗВАНИЕ (2-4 слова) и описание.
...
... и так до 9.`
    ...
    console.log('✅ Задания сгенерированы через Yandex GPT');
    return tasks;
    
  } catch (error) {
    console.error('❌ Ошибка Yandex GPT:', error.message);
